...
class KerasPilot(ABC):
        ...
        if hasattr(self.model, 'inputs'):
            for inp in self.model.inputs:
                if inp.name.split('/')[0].split(':')[0] == input_name:
                    return inp.shape
    # または代替として
        return self.model.get_layer(input_name).input_shape
    ...
    def train(self,
        ...
        gpus = tf.config.experimental.list_physical_devices('GPU')
        if gpus:
            logger.info(f"Training with {len(gpus)} GPU(s)")
        else:
            logger.info("Training with CPU")
        tic = datetime.datetime.now()
        logger.info('////////// Starting training //////////')
        workers = 4 if gpus else 1
        print(model_path)
        ...
        formatted = f"{now.month}-{now.day}_{now.hour}-{now.minute}-{now.second}"
        print("now time :",formatted)
        model_path = "/content/mycar/models/" + formatted + "_keras" + ".keras"
        print("new_model_path :",model_path)
        model.save(model_path)
        model_path_h5 = "/content/mycar/models/" + formatted + "_h5" + ".h5"
        model.save(model_path_h5)
        # Kerasモデルをロード（または訓練直後のモデル）
        model_tflite = tf.keras.models.load_model(model_path)  # もうロード済みならこの行はスキップでOK！
        print("model_tflite :",model_tflite)
# TFLiteConverterで変換
        converter = tf.lite.TFLiteConverter.from_keras_model(model_tflite)
        tflite_model = converter.convert()

# .tfliteファイルとして保存
        tf_model_name = "/content/mycar/models/" + formatted + "_tfmodel"+ ".tflite"
