...
Usage:
    ...
    manage.py (tune)  [--tubs=tubs] (--model=<model>) [--type=(linear|inferred|tensorrt_linear|tflite_linear)] [--try=max trial]
...
def tune(cfg, tub_names, model_name, type="linear", trials=20):
    ...
    print(f"{trials}回のトライアルで{type}モデルをチューニングします")
    
    # Tubの設定 - tub_namesが文字列であることを確認
    if isinstance(tub_names, list):
        tub_names = ','.join(tub_names)
    
    # モデルパスを展開
    model_path = os.path.expanduser(model_name)
    
    # OptunaObjectiveクラスを定義
    class OptunaObjective:
        
        def __init__(self, cfg, tub_names, model_type):
            self.cfg = cfg
        ...
        def __call__(self, trial):
            ...
            # 一時的なモデルパス
            temp_model_path = f"{model_path}_trial_{trial.number}.keras"
            # 訓練設定を更新
            import copy
            cfg_local = copy.deepcopy(self.cfg)
            cfg_local.BATCH_SIZE = batch_size
            cfg_local.EARLY_STOP_PATIENCE = patience
            cfg_local.OPTIMIZER = optimizer_type
            cfg_local.LEARNING_RATE = learning_rate
            cfg_local.LEARNING_RATE_DECAY = weight_decay
            
            try:
                # 標準のDonkeycar訓練関数を使用
                # tub_namesは文字列として渡す
                print(f"////////////  トライアル {trial.number} の訓練を開始します  ////////////")
                print("今回のパラメータです")
                data = {
                    'tag': ['batch_size', 'patience', 'optimizer_type','learning_rate','weight_decay'],
                    'value': [batch_size, patience, optimizer_type, learning_rate, weight_decay],
                    }
                df = pd.DataFrame(data)
                print(df.to_string(index=False))
                print("///////////////////////////////////////////////////////////////////////")
                hist = train(cfg_local, self.tub_names, temp_model_path, 
                           model_type=self.model_type, transfer=None, 
                           comment=f"optuna_trial_{trial.number}")
                ...
                print(f"トライアル {trial.number} でエラーが発生しました: {e}")
                # エラーが発生した場合は高い値を返す
                return float('inf')
    ...
    # 目的関数
    objective = OptunaObjective(cfg, tub_names, type)
    
    # 最適化を実行
    print(f"Optunaの最適化を開始します（{trials}回のトライアル）...")
    trials = float(trials)
    ...
    print(f"最適な検証損失: {study.best_value}")
    
    # 最適なパラメータで最終モデルを訓練
    print("\n最適なハイパーパラメータで最終モデルを訓練しています...")
    
    # 設定を更新
    cfg.BATCH_SIZE = study.best_params["batch_size"]
    cfg.EARLY_STOP_PATIENCE = study.best_params["patience"]
    cfg.OPTIMIZER = study.best_params["optimizer"]
    cfg.LEARNING_RATE = study.best_params["learning_rate"]
    cfg.LEARNING_RATE_DECAY = study.best_params["weight_decay"]
    
    # 最終モデルの訓練 - tub_namesを文字列として渡す
    train(cfg, tub_names, model_path, model_type=type, transfer=None, comment="optuna_best")
    
    # 最適化結果の保存
    results_path = os.path.splitext(model_path)[0] + "_optuna_results.txt"
    with open(results_path, 'w') as f:
        f.write("最適なハイパーパラメータ:\n")
        for key, value in study.best_params.items():
            f.write(f"  {key}: {value}\n")
        f.write(f"\n最適な検証損失: {study.best_value}\n")
    
    # 最適化の履歴とパラメータ重要度のプロット
    try:
        plt.figure(figsize=(10, 6))
        optuna.visualization.matplotlib.plot_optimization_history(study)
        plt.title("optuna_history")
        plt.savefig(os.path.splitext(model_path)[0] + "_optuna_history.png")
        
        plt.figure(figsize=(10, 6))
        optuna.visualization.matplotlib.plot_param_importances(study)
        plt.title("optuna_importances")
        plt.savefig(os.path.splitext(model_path)[0] + "_optuna_importances.png")
        
        print("最適化グラフを保存しました")
    except Exception as e:
        print(f"グラフ作成中にエラーが発生しました: {e}")
    
    print(f"最適化されたモデルを保存しました: {model_path}")
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if cfg.HAVE_IMU:
        ...
        V.add(imu, outputs=['imu/acl_x', 'imu/acl_y', 'imu/acl_z',
            'imu/gyr_x', 'imu/gyr_y', 'imu/gyr_z','imu/one_hot_state_array'], threaded=True)
    ...
    class DriveMode:
        def run(self, mode,
            ...
            #elif imu_one_hot_state_array[1]==1 :
                #exit()
...
if __name__ == '__main__':
    ...
    if args['drive']:
        ...
        tune(cfg, tub_names=args['--tubs'], model_name=args['--model'], type=args['--type'], trials=args['--try'])
